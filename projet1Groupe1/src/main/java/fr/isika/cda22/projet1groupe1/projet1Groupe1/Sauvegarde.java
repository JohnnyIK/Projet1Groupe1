...
public class Sauvegarde extends Annuaire implements ParametreGestionnaire {
	...
	// Méthode pour afficher dans la console le contenu du fichier binaire
		...
		/**
		 * Affiche dans la console le contenu du fichier binaire dont l'adresse est passée en argument
		 * @param cheminFichierBin Le chemin du fichier binaire
		 ...
		 */
		public void lireSauvegardeStagiaire(String cheminFichierBin) {
			try {
				System.out.println("\n******* FICHIER BIN: "+cheminFichierBin);
				RandomAccessFile raf = new RandomAccessFile(cheminFichierBin, "rw");
				int nbrStagiaires = (int) (raf.length() / (TAILLE_NOEUD_OCTET));
				// on ouvre le flux "raf" associé au fichier binaire
